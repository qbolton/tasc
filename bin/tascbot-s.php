...
define('OPTSHARE_TERMEXTRACT','/opt/share/term-extraction/TermExtractor');
...
class TascBot {
  ...
  public function startup( $feed_id = 0, $feed_limit = 0 ) {
    ...
    if (count($results) == 1) {
      $log_level = $results[0]->job_log_level;
      ...
      if (strcasecmp($log_level,'ERROR') == 0) {
        $this->site->log_level = Monolog\Logger::ERROR;
      }
      else if (strcasecmp($log_level,'DEBUG') == 0) {
        $this->site->log_level = Monolog\Logger::DEBUG;
      }
      else if (strcasecmp($log_level,'WARNING') == 0) {
        $this->site->log_level = Monolog\Logger::WARN;
      }
      else if (strcasecmp($log_level,'INFO') == 0) {
        $this->site->log_level = Monolog\Logger::INFO;
      }
      else {
        $this->site->log_level = Monolog\Logger::ERROR;
      }

      $this->logger->pushHandler( new Monolog\Handler\StreamHandler($results[0]->job_logfile, $this->site->log_level) );
  ...
  public function run( $feed_id = 0 ) {
    ...
    // setup items_to_post array
    // this is an array of arrays containing all of the items to post at the end of parsing.
    // once the array is full it will be looped over and all of it's post content will be saved to wordpress
    // in batch mode.
    $items_to_post = array();

    // get the feeds necessary to run with
    $feed_list = $this->getFeeds( $feed_id, $this->site->feed_limit ); //print_r($feed_list);
    ...
    foreach ($feed_list as $feed) {
      ...
      $feed_handler = NULL;
      $feed_items = NULL;
      $fetch_status = "SUCCESS ON FETCH";

      // only modified by tascbot global modules
      $process_counts->global_module_exclude = 0;
      // only modified by feed class handler functions
      $process_counts->feed_handler_exclude = 0;

      // =============================================================
      // CREATE AUTHOR
      // =============================================================
      list($author_id, $author_status) = $this->wordpress_author($feed->feed_name,$feed->feed_name,$feed->web_url);
      ...
      try {
        ...
        // set timeout
        $sp->set_timeout(7);
        // set cache duration (8 hours)
        $sp->set_cache_duration(28800);
        // set cache location
        $sp->set_cache_location($this->site->cachedir);
        // spoof browser hit
        $sp->set_useragent($this->site->useragent);
        ...
        $sp = NULL;
        $fetch_status = "SIMPLEPIE CRITICAL EXCEPTION";
        $this->logger->error("<<<>>> {$e->getMessage()} ");
      }

      // if there was a problem with this feed, dump log message and continue
      if (!empty($sp->error)) {
        $this->logger->error("<<<>>> {$sp->error} ");
        $fetch_status = "SIMPLEPIE XML EXCEPTION";
        $sp = NULL;
      }

      // =============================================================
      // UPDATE FETCH DATE AND STATUS
      // =============================================================
      DB::table($this->site->feedtable)->where('fid',$feed_id)->update(['fetch_status'=>$fetch_status,'fetch_date'=>DB::raw('NOW()')]);
      if (strcasecmp($fetch_status,'SUCCESS ON FETCH') !=0) {
        // skip to next feed in the array
        continue;
      }

      // put feed items into array
      if ($feed->fetch_cap > 0) {
        $feed_items = $sp->get_items(0,$feed->fetch_cap);
      }
      else {
        // make sure we never grab more than 10 items from any single feed, it's too much otherwise
        $feed_items = $sp->get_items(0,10);
      }

      $process_counts->fetched += count($feed_items);
      // clear that big ole simplepie class out of mem
      $sp = NULL; //print_r(count($feed_items)); 

      // if successful fetch, then proceed

      // =============================================================
      // Loop Over Incoming Feed Items
      // =============================================================

      // to make sure $feed_item is always an array, force it into one
      if (! is_array($feed_items) ) { $feed_items = array($feed_items); }

      foreach ($feed_items as $item) {
        $item_link = $item->get_link();

        $this->logger->info(">>> " . $item_link . " will be content extracted"); 

        // =============================================================
        // call to Embedly wrapper method with cache support
        // =============================================================
        try {
          // check to see if caching enabled
          //$file_date = date("mdy"); 
          $file_date = strtotime( date("l") );
          $cache_filename = $this->site->cachedir . 
            $file_date . '-' . $this->site->job_id . '-' . $feed_id . '-' . md5(trim($item_link)) . '.cache';

          // check to see if cache data exists for url
          if (file_exists($cache_filename)) {
            // read the file in
            $cache_string = file_get_contents($cache_filename);
            if ($cache_string !== FALSE) {
              $content = json_decode($cache_string);
              if ( (! is_object($content)) || (is_null($content))) {
                // pop off an error
                throw new Exception("CACHE FILE EMPTY and we can't read it.  We'll have to skip");
              }
              else {
                $this->logger->info(">>> CACHE READ from {$cache_filename}");
              }
            }
            else {
              $this->logger->warning(">>> CACHE READ FAIL {$cache_filename}");
            }
          }
          else {
            // build full text rss request url
            $extract_request = 'http://dev.elombre.com/ftr/extract.php?url=' . urlencode($item_link) . '&xss=0&parser=html5php';
            // make the request and fetch results
            $content = Fetch::withCurl('GET',$extract_request);
            // decode json results
            $content = json_decode($content->contents); //print_r($content);

            // write the content to a cache file
            $cache_write = file_put_contents($cache_filename,json_encode($content));
            if ($cache_write !== FALSE) {
              $this->logger->info(">>> CACHE WRITE {$cache_filename}");
            }
            else {
              $this->logger->info(">>> CACHE WRITE FAIL {$cache_filename}");
            }

            // grab the full text request body
            //$ftr_request = sprintf('%s?url=%s&format=%s&html=%s&exc=%s&parser=html5php&summary=%s',
            //  'http://dev.elombre.com/ftr/makefulltextfeed.php',urlencode($item_link),'json',1,1,1);
            //$full_text = Fetch::withCurl('GET',$ftr_request); 
            //$full_text = json_decode($full_text->contents);
            // if present, add the content_encoded 
            //print_r($full_text); exit;
          }
        }
        catch(Exception $e) {
          $this->logger->error("<<<>>> {$e->getMessage()} ");
        ...
        // =============================================================
        // ADD ELEMENTS AND PREPARE TO PARSE RSS AND EMBEDLY CONTENT
        // =============================================================
        
        /* We should probably add the pubdate exclusion check right here */
        $content->site_id = $site_id;
        $content->fid = $feed_id;

        // add in publication date
        $content->pubDate = $item->get_date();
        ...
        $this->logger->info('>>> Parsing ' . $item_link . ' content info correct fields');

        // parse the content objects into post fields
        $ep = new ExtractParser_S( $item, $content ); //print_r($item->get_content()); 
        // squash the item and content
        $item = NULL; $content = NULL;

        // process and hopefully return a use and valid post item
        $post = $ep->parse(); //print_r($post);  exit;

        $process_counts->parsed++;

        // =============================================================
        // RUN ANY CUSTOM FEED HANDLERS FOR THE JOB
        // =============================================================
        if ( (! is_null($feed->feed_class) ) && (strlen($feed->feed_class)>3) ) {
          $feed_class = strtolower($feed->feed_class);
          $this->logger->info('>>> FIRING FEED_CLASS ' . $feed_class);
          try {
            if (class_exists($feed_class)) {
              $fc_obj = new $feed_class ($post,$feed);
              $post = $fc_obj->run(); 
            }
            else {
              throw new Exception("FEED_CLASS {$feed_class} DOES NOT EXIST");
            }
          }
          catch (Exception $e) {
            $this->logger->error("<<<>>> {$e->getMessage()} ");
        ...
        // =============================================================
        // DETERMINE IF POST SHOULD BE EXCLUDED
        // =============================================================
        $exclude_result = $this->isExcluded( $post, TRUE, $feed, $this->site->parameters );
        if ($exclude_result->value == FALSE) {

          // =============================================================
          // IF ALL OF THE ABOVE GOES WELL, ADD TO POST ITEMS BUFFER 
          // =============================================================

          // if good author status then commence with the save
          if ( $author_status ) {

          //$this->map_categories($post,$feed,$tags);
          // add the mapped items to $post
          $post->author_id = $author_id;

          // wordpress post builds the post array and returns it
          $items_to_post[] = $this->wordpress_post($post,$feed->feed_tags,$feed->feed_categories); 
          //print_r($post); exit;
          }
        }
        else {
          $process_counts->excluded++;
          $process_counts->feed_exclude++;
          /* if the exclude reason was due to the item being too old, then skip the remaining items, 
          they'll also be too */
          if (strcasecmp($exclude_result->reason,"ITEM_AGE_EXCLUDE") == 0) {
            $this->logger->warning(">>> SKIPPING remaining items for {$feed->feed_name} as they are also likely too old");
            // break out of loop
            break;
          }
          // too many skips check
          else if ($process_counts->feed_exclude > 5) {
            $process_counts->feed_exclude = 0;
            $this->logger->warning(">>> TOO MANY SKIPS for {$feed->feed_name} to continue ");
            // break out of loop
            break;
          }
        } // END IS EXCLUDED 
      } // END LOOP OVER INDIVIDUAL FEED ITEMS
    } // END LOOP OVER FEED LIST

    // =============================================================
    // SAVE THE ITEMS TO POST
    // =============================================================
    // check to see if there are any items in the $items_to_post array and save them
    if (count($items_to_post) > 0) {
      $process_counts->posted = $this->wordpress_save($items_to_post);
    }
    else {
      $this->logger->warning(">>> There are no items to post");
    }
   
    // print save report
    $this->logger->alert(">>> CLOSING REPORT> " . 
      "{$process_counts->fetched} fetched, " .
      "{$process_counts->parsed} parsed, " .
      "{$process_counts->excluded} excluded, " .
      "{$process_counts->posted} posted "
    );
  ...
  public function wordpress_save($array_of_posts,$meta_keys=NULL) { 
    ...
    foreach($array_of_posts as $post) {
      ...
      $meta = array(
        ...
        'post_score' => ((isset($post->post_score)) ? $post->post_score : 0) 
      );
      ...
      if ($wp_post_id > 0) {
        ...
        add_post_meta($wp_post_id, 'post_score', $meta['post_score'], TRUE);
